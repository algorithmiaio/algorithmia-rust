...
pub fn entrypoint(_args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let item = parse_item(&input.to_string())
        .expect("#[entrypoint] must be attached to a function");

    let entrypoint = Entrypoint::from_item(item);
    entrypoint.output().parse().expect("#[entrypoint] failed to parse output")
}
    ...
    fn from_item(item: Item) -> Self {
        ...
        match item.node {
            ItemKind::Fn(fn_decl, unsafety, _constness, _abi, generics, _block) => {
                assert_eq!(unsafety, Unsafety::Normal, "Unsafe #[entrypoint] functions are not supported");
                assert_generics_empty(&generics);

                assert_eq!(fn_decl.inputs.len(), 1, "#[entrypoint] function may only take one arg");
                let input_type = match fn_decl.inputs[0] {
                    FnArg::SelfRef(..) | FnArg::SelfValue(..) => panic!("Attach #[entrypoint] to impl block to support &self arg"),
            ...
            ItemKind::Impl(unsafety, _polarity, generics, _path, ty, impl_items) => {
                assert_eq!(unsafety, Unsafety::Normal, "Unsafe #[entrypoint] impls are not supported");
                assert_generics_empty(&generics);
                // TODO: assert that `ty` isn't of type `Algo` which would cause name collisions

                let method_name = Ident::new("apply");

                let fn_decl = impl_items.iter()
                    .filter_map(|item| {
                        match item.node {
                            ImplItemKind::Method(ref sig, _) => {
                                Some((&item.ident, &sig.decl))
                            }
                            _ => None
                        }
                    })
                ...
                assert_eq!(fn_decl.inputs.len(), 2, "#[entrypoint] within an impl must take &self and an input arg");
                let input_type = match fn_decl.inputs[1] {
                    FnArg::SelfRef(..) | FnArg::SelfValue(..) => panic!("Are you using self as a second argument?"),
                    FnArg::Captured(_, ref ty) => ty.clone(),
                    FnArg::Ignored(ref ty) => ty.clone(),
                };
